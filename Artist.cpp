...
void loadFontTextures(Artist::fontTextureSet* fontToLoad, std::string fontName) //HEEEEEEEEELP
{
	Artist artist;
...
std::vector<SDL_Texture*> Artist::loadAnimationData(std::string textureFolderLocation, std::string textureSetName, int textureAmount)
{
	Artist artist;
...
bool Artist::loadMedia()
{
	...
	//TEMP
	dirt = loadTexture("Resource/tiles/dirt.png");
...
void Artist::drawImageChunk(SDL_Texture* tex, int x, int y, SDL_Rect chunk, int w, int h, double r, SDL_Point* center, SDL_RendererFlip flip, bool scaled)
{
	SDL_Rect img;
	img.x = x * Artist::SCREEN_SCALE;
	img.y = y * Artist::SCREEN_SCALE;
	...
	if (scaled)
		...
		img.h = drawH;
	SDL_RenderCopyEx(gRenderer, tex, &chunk, &img, r, center, flip);
}

void Artist::drawAnimation(std::vector < SDL_Texture*> texSet, int x, int y, int width, int height, int start, float rot, int frameTime, SDL_RendererFlip flip)
{
	int frameToDraw = (clock() - start) / frameTime % texSet.size();
	//printf("FrameTime: %i, DisplayedFrame: %i \n", clock(), frameToDraw);
	drawImage(texSet[frameToDraw], x, y, width, height, rot, 0, flip);
}
