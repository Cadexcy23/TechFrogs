...
public:
	...
	static const Uint32 FRAME_TIME = 100;
	...
	static SDL_Texture* loadTargetTexture(int w, int h);

	static std::vector<SDL_Texture*> loadAnimationData(std::string textureFolderLocation, std::string textureSetName, int textureAmount);

	static void drawImage(SDL_Texture* tex, int x, int y, int w = 0, int h = 0, double r = 0, SDL_Point* center = NULL, SDL_RendererFlip flip = SDL_FLIP_NONE, bool scaled = true);

	static void drawImageChunk(SDL_Texture* tex, int x, int y, SDL_Rect chunk, int w = 0, int h = 0, double r = 0, SDL_Point* center = NULL, SDL_RendererFlip flip = SDL_FLIP_NONE, bool scaled = true);

	static void drawAnimation(std::vector<SDL_Texture*> texSet, int x, int y, int width = 0, int height = 0, int start = 0, float rot = 0, int frameTime = FRAME_TIME, SDL_RendererFlip flip = SDL_FLIP_NONE);
	...
	static void changeRenderColor(int r, int g, int b, int alpha);

	static void drawLineFromPoints(int sX, int sY, int eX, int eY);
